...
if __name__ == "__main__":
    # Bot alag thread mein
    threading.Thread(target=run_bot, daemon=True).start()

    # Flask main thread mein (Render ke liye)
    port = int(os.environ.get("PORT", 10000))
    print(f"Flask server starting on port {port}")
    app.run(host="0.0.0.0", port=port)        response.headers.add("Access-Control-Allow-Headers", "Content-Type")
        response.headers.add("Access-Control-Allow-Methods", "POST")
        return response
...
if __name__ == "__main__":
    ...
    # Flask ko main thread mein chalao (Render ke liye zaroori)
    port = int(os.environ.get("PORT", 10000))
    print(f"Flask server starting on port {port}")
    app.run(host="0.0.0.0", port=port)        if not any(x in url for x in ["instagram.com", "facebook.com", "fb.watch", "fb.com"]):
            return jsonify({"status": "error", "message": "Only Instagram Reels & Facebook Videos supported"}), 400
